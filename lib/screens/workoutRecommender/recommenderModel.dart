...
class RecommenderModel {
  ...
  factory RecommenderModel.fromJson(Map<dynamic, dynamic> json) {
    Workout recommendedWorkout = Workout(name: json["name"].toString(), exercises: []);

    for (int index = 0; index < json["exercises"].length; index++) {
      Exercise exercise = Exercise(name: json["exercises"][index]["name"].toString(), sets: []);
      for (int i = 0; i < json["exercises"][index]["sets"]; i++) {
        exercise.addSet(json["exercises"][index]["weight"].toDouble(), json["exercises"][index]["reps"]);
      }
